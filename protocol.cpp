...
void SRHelper::sendMsg(QTcpSocket *sock, uint16_t msgType, const std::string &content)
{
...
void SRHelper::sendMsg(QTcpSocket *sock, VioletProtHeader header, const std::string &content) {
    Msg msg;
    msg.header = header;
    msg.neck = {};
    msg.content.assign(content.begin(), content.end());
...
void SRHelper::sendMsg(QTcpSocket *sock, VioletProtNeck neck, const std::string &content) {
    Msg msg;
    msg.header.magic = htonl(0x43484154); // "CHAT"
    msg.header.version = htons(1);
...
void SRHelper::sendMsg(QTcpSocket *sock,
...
std::optional<Msg> SRHelper::recvMsg(QTcpSocket *sock, ssize_t byteToRead)
{
    VioletProtHeader header;
    VioletProtNeck neck;
    qint64 len;
