...
UserMainWindow::UserMainWindow(QTcpSocket *socket, std::string uname, std::vector<std::string> iuserfriend, std::vector<std::string> iusergroup, QWidget *parent) :
    QWidget(parent),
...
void UserMainWindow::closeEvent(QCloseEvent *ev)
{
    if (1) { // 自定义条件检查
        ...
        emit sclose();
        disconnect(sock, &QTcpSocket::readyRead, this, &UserMainWindow::read_cb);
        this->deleteLater();
        ev->accept();
    } else {
        ev->ignore(); // 阻止关闭
